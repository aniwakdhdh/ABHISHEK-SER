async function handler(m, {usedPrefix, command}) {
  ...
  switch (command) {
    ...
    case 'leave': {
      ...
      if (!room) return this.sendMessage(other, {text: `*DO NOT STAY IN AN ANONYMOUS CHAT*\n\n*DO YOU WANT TO START ONE?*\nWrite ${usedPrefix}start`}, {quoted: m});
      ...
      if (other) await this.sendMessage(other, {text: `*THE OTHER USER AH LEAVES THE ANONYMOUS CHAT*\n\n*DO YOU WANT TO GO TO Another ANONYMOUS CHAT?*\nWrite ${usedPrefix}start`}, {quoted: m});
      // this.sendButton(other, '*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝙻 𝙾𝚃𝚁𝙾 𝚄𝚂𝚄𝙰𝚁𝙸𝙾 𝙰𝙷 𝙰𝙱𝙰𝙽𝙳𝙾𝙽𝙰𝙳𝙾 𝙴𝙻 𝙲𝙷𝙰𝚃 𝙰𝙽𝙾𝙽𝙸𝙼𝙾*\n\n*¿𝚀𝚄𝙸𝙴𝚁𝙴𝚂 𝙸𝚁 𝙰 𝙾𝚃𝚁𝙾 𝙲𝙷𝙰𝚃 𝙰𝙽𝙾𝙽𝙸𝙼𝙾?*\n_𝙳𝙰 𝙲𝙻𝙸𝙲𝙺 𝙴𝙽 𝙴𝙻 𝚂𝙸𝙶𝚄𝙸𝙴𝙽𝚃𝙴 𝙱𝙾𝚃𝙾𝙽_', author, null, [['𝙸𝙽𝙸𝙲𝙸𝙰𝚁 𝙲𝙷𝙰𝚃 𝙰𝙽𝙾𝙽𝙸𝙼𝙾', `.start`]], m)
      delete this.anonymous[room.id];
      if (command === 'leave') break;
    }
    case 'start': {
      if (Object.values(this.anonymous).find((room) => room.check(m.sender))) return this.sendMessage(m.chat, {text: `*You Are Still In Anonymous Chat. Unchat Or Wait For Another User To Join To Start*\n\n*¿𝚀𝚄𝙸𝙴𝚁𝙴𝚂 𝚂𝙰𝙻𝙸𝚁 𝙳𝙴𝙻 𝙲𝙷𝙰𝚃 𝙰𝙽𝙾𝙽𝙸𝙼𝙾?*\n𝙴𝚂𝙲𝚁𝙸𝙱𝙴 ${usedPrefix}leave`}, {quoted: m});
